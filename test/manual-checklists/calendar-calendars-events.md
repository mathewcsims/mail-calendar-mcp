...
| 4 | Calendar from #2 | `calendar_delete_calendar` with `confirm:true` | Succeeds via the EventKit backend (calendar gone from Calendar.app). If forced to JXA (`MCM_CALENDAR_BACKEND=jxa`), expect `PLATFORM_LIMITATION` | [ ] |
...
| 11 | A **recurring** event (has `recurrence` set) | `calendar_delete_event` with `confirm:true` | Whole series actually deleted via the EventKit backend (verify in Calendar.app). Also test `occurrenceDate` + `span:"this"` deletes just one occurrence. If forced to JXA, expect `PLATFORM_LIMITATION` (confirmed bug: reports success but doesn't delete) | [ ] |
